...
func TestReviewCard(t *testing.T) {
	...
	reviewAndCheck := func(t *testing.T, rating int, expectedState db.CardState, expectedInterval time.Duration, expectedEase float64, expectedNextReview time.Time, expectedReviewCount int, expectedLapsCount int) {
	...
	reviewAndCheck(t, 3, db.StateLearning, 10*time.Minute, 2.5, now.Add(10*time.Minute), 1, 0)

	// Step 2: Learning step 2, Good (graduates to Review)
	now = now.Add(10 * time.Minute)
	reviewAndCheck(t, 3, db.StateReview, 24*time.Hour, 2.5, now.Add(24*time.Hour), 2, 0)

	// Step 3: Review, Good (interval = 4 days)
	//now = now.Add(24 * time.Hour)
	//reviewAndCheck(t, 3, db.StateReview, 4*24*time.Hour, 2.5, now.Add(4*24*time.Hour), 3, 0)

	//// Step 4: Review, Good (interval = 4 × 2.5 = 10 days)
	//now = now.Add(4 * 24 * time.Hour)
	//reviewAndCheck(t, 3, db.StateReview, 10*24*time.Hour, 2.5, now.Add(10*24*time.Hour), 4, 0)
	//
	//// Step 5: Review, Again (moves to Relearning)
	//now = now.Add(10 * 24 * time.Hour)
	//reviewAndCheck(t, 1, db.StateRelearning, time.Minute, 1.7, now.Add(time.Minute), 5, 1)
	//
	//// Step 6: Relearning, Good (graduates back to Review)
	//now = now.Add(time.Minute)
	//reviewAndCheck(t, 3, db.StateReview, 24*time.Hour, 1.7, now.Add(24*time.Hour), 6, 1)
	//
	//// Step 7: Review, Easy (interval = 1 × 1.7 × 1.3 ≈ 2 days)
	//now = now.Add(24 * time.Hour)
	//reviewAndCheck(t, 4, db.StateReview, 2*24*time.Hour, 1.85, now.Add(2*24*time.Hour), 7, 1)
}
