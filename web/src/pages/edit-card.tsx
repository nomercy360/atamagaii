...
export default function EditCard() {
  ...
  const [cardFields, setCardFields] =
    createSignal<CardFields>(defaultCardFields)

  // Fetch the card data when the component mounts
  const [card] = createResource<Card | null>(async () => {
  ...
  return (
    <div class="container mx-auto px-4 py-6 max-w-lg h-screen overflow-y-auto pb-20">
      ...
      <Show when={error()}>
        <div class="bg-error/10 text-error p-3 rounded-md mb-4">{error()}</div>
      </Show>
        ...
        >
          ...
          <div class="space-y-4">
            ...
            <div>
              <label
                class="block text-sm font-medium mb-1"
                for="example_with_transcription"
              >
              ...
              <textarea
                ...
                onInput={e =>
                  handleInputChange(e, 'example_with_transcription')
                }
                class="bg-secondary w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary min-h-[80px]"
              />
            </div>
