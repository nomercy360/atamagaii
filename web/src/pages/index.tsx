...
import { Deck, apiRequest } from '~/lib/api'
...
export default function Index() {
  ...
  const getTotalCards = (deck: Deck) => {
    ...
    return (
      deck.stats.review_cards + deck.stats.new_cards + deck.stats.learning_cards
    )
  }
    ...
    <div class="container mx-auto px-4 py-6 max-w-md flex flex-col items-center overflow-y-auto h-screen pb-28">
      <div class="w-full">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-medium">{t('home.deck')}</h3>
        </div>
        <div class="space-y-2">
          <Show
            when={!decksQuery.isPending}
            fallback={<DeckSkeleton />}
          >
            <For each={decksQuery.data}>
              {deck => (
                <div class="bg-card w-full text-card-foreground p-4 rounded-xl transition-colors flex justify-between items-center">
                  <button
            ...
            {decksQuery.data && decksQuery.data.length === 0 && (
              <div class="text-center py-8">
                <p class="text-muted-foreground mb-4">{t('home.no_decks')}</p>
                <button
